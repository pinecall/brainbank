...
### Removed
- **`brainbank_index` tool** — indexing is now CLI-only (`brainbank index`)

### Changed
- **`repo` param now required** on `brainbank_context` and `brainbank_files` — forces AI tools to always specify the project root

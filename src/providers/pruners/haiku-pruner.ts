 ...
 * "relevant" or "noise" based on filePath, metadata, and a
...
export class HaikuPruner implements Pruner {
    ...
    async prune(query: string, items: PrunerItem[]): Promise<number[]> {
        ...
        const prompt =
            `Query: "${query}"\n\nSearch results:\n${itemLines}\n\n` +
            `You are a search result filter. Return a JSON array of #IDs to KEEP.\n` +
            `ONLY drop files that are CLEARLY UNRELATED to the query (e.g. a logging transport, a user profile component, a generic app shell).\n` +
            `KEEP any file that could plausibly be related — even indirectly (entities, types, configs, stores, settings, modules, etc.).\n` +
            `When in doubt, KEEP the result.\n` +
            `Example response: [0, 1, 2, 5, 8]`;

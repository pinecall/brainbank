...
- **Interactive Stats TUI** (`brainbank stats`) — full Ink-based explorer replacing the plain-text output. 4 views: Dashboard (overview, language bars, directory list), File Explorer (drill into dirs with sortable file list + detail panel showing symbols, imports, call edges), Chunk Viewer (browse chunks with line-numbered content preview + call graph info), Call Graph (interactive symbol search + expandable call tree). Supports `--repo` for cross-repo exploration. Old output preserved via `--plain` flag. Uses `node:sqlite` read-only connections for zero-overhead data access

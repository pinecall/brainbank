...
- **Interactive Stats TUI** (`brainbank stats`) — full Ink-based explorer with 5 views: Dashboard (overview, language bars, directory list, `/` search shortcut), File Explorer (sortable file list + detail panel), Chunk Viewer (Tab-switchable split panel with scrollable code preview — `↑↓/u/d` to scroll, `h/l` or Tab to switch focus, scroll % indicator), Call Graph (symbol search + expandable call tree), **Search** (full-text search across chunk content/names/paths with live preview + `{/}` scroll). Supports `--repo`, `--plain`. Uses `node:sqlite` read-only connections. Clears terminal and uses 90%×90% sizing for full-screen feel

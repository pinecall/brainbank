...
- **Third-party plugin discovery** — `loadPlugin()` now falls back to `import(name)` for npm packages not in the built-in registry. Third-party plugins (e.g. `brainbank-csv`) can be listed in `config.json` `plugins` array and are auto-loaded by the CLI
- **Interactive Stats TUI** (`brainbank stats`) — full Ink-based explorer with 5 views: Dashboard, File Explorer, Chunk Viewer (Tab-switchable split panel with scrollable code preview), Call Graph, and **Semantic Search** — full BrainBank pipeline (vector→prune→expand) in a 3-column layout: Raw results with score%, Pruned/Ranked with reorder position + file paths (dropped items shown as strikethrough in Raw), and a wide scrollable code Preview. Source filter tabs auto-discovered from plugins (1-4 to toggle). Pipeline timings bar shows per-stage ms + counts. Enter-to-search for API cost control. Lazy brain initialization on first `/`. Supports `--repo`, `--plain`. Uses `node:sqlite` for static views, full `BrainBank` instance for search
- **TUI integration for external plugins** — plugins can export optional `scan(repoPath)` and `preview(repoPath)` functions for TUI sidebar and explorer panel integration. New types `PluginScanInfo` and `PluginPreviewLine` exported from core. TUI explorer panel is now fully dynamic — renders previews from any plugin, not just built-in code/docs/git
- **Auto-detect docs collections** — when `docs` plugin is selected during setup, the CLI now auto-detects common doc folders (`docs/`, `doc/`, `wiki/`, `documentation/`, `guides/`, `notes/`) and adds them as collections to `config.json`. Previously the docs plugin silently indexed 0 documents without explicit `docs.collections` config
- **Syntax highlighting in Stats TUI** — code previews in Chunk Viewer and Semantic Search now have basic syntax highlighting: keywords (purple), strings (green), numbers (orange), types/PascalCase (cyan), decorators (yellow), comments (dim). Regex-based with string-aware comment detection
- **Search loading modal** — entering search mode (`/`) now shows an animated spinner modal while the BrainBank search engine initializes (HNSW indices + plugins). Deferred init via `setTimeout` ensures the modal renders before heavy sync work blocks the event loop
- **Persistent search session** — `BrainSearchSession` is created once in `StatsApp` and survives view transitions. Pressing Esc then `/` again is instant — no re-initialization
- **File Explorer filter** (`/`) — in `bb stats` file explorer, press `/` to filter files by name. Shows filtered count (`12/150`), Esc clears filter, arrows navigate filtered results
- **Config pattern preservation** — `generatePatternsFromTree` now accepts original include patterns and preserves scoped patterns like `packages/mongo/src/**` instead of flattening to `packages/mongo/**`. Individual file patterns (e.g. `apps/core/new_server.py`) also preserved across `--setup` runs
...
- **Multi-repo support — completely removed** — deleted `detectGitSubdirs()`, `isMultiRepoCapable()`, `MULTI_REPO_PLUGINS`, `scanGitSubdirs()`, `readReposFromConfig()`, `_detectRepoPrefix()`, colon-separated plugin naming (`code:frontend`), sub-repo path prefixing in search results, and type-prefix matching in `PluginRegistry` (`allByType`, `firstByType`). Plugin names are now always simple strings (`code`, `git`, `docs`). Cleaned `builtin-registration.ts`, `context-builder.ts`, `composite-vector-search.ts`, `composite-bm25-search.ts`, `index-api.ts`, `watch.ts`, `plugin-registry.ts`, `scan.ts`, `index.ts`, and both `@brainbank/code` and `@brainbank/git` plugin options. Deleted `docs/multi-repo.md`. Removed all multi-repo references from 10+ doc files
...
- **Removed circular `brainbank` self-dependency** from devDependencies — prevented npm from pulling the published version (with `better-sqlite3`) into `node_modules`
